...
def calculate_data_range(points3d):
    ...
    """
    x_percentiles = np.nanpercentile(points3d[:, 0], [20, 50, 80])
    x_diff = x_percentiles[-1]-x_percentiles[0]
    y_percentiles = np.nanpercentile(points3d[:, 1], [20, 50, 80])
    y_diff = y_percentiles[-1]-y_percentiles[0]
    z_percentiles = np.nanpercentile(points3d[:, 2], [20, 50, 80])

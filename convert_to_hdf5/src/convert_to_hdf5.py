...
def load_hdf_files(record_names, out_dir, data_info_mapping, meta_data, topic_info):
    ...
    for idx, hdf5_fn in enumerate(record_names):
        ...
        try:
            hdf5_files[idx] = h5py.File(hdf5_fn_full, 'w')
        except:  # pylint: disable=bare-except
            rospy.logerr(
        ...
        for vid_topic in data_info_mapping:
            ...
            rospy.loginfo(
                'Creating new Datasets for standard topic : %s' % (vid_topic))
            rospy.loginfo(
...
def node():
    ...
    rospy.loginfo('output director: %s', out_dir)
    ...
    bridge = CvBridge()
    for vid_topic in tqdm(data_info_mapping):
